	...
	PF_LayerDef		*output )
{
	if (in_dataP->appl_id == 'PrMr')
	{
		...
		float brightness = (float)FIX_2_FLOAT(params[SDK_INVERT_PROCAMP_EXPOSURE]->u.fd.value) / 100.0f;
	...
	PF_PreRenderExtra	*extraP)
{
	...
	if (infoP) {
		...
		ERR(PF_CHECKOUT_PARAM(in_dataP, SDK_INVERT_PROCAMP_EXPOSURE, in_dataP->current_time, in_dataP->time_step, in_dataP->time_scale, &cur_param));
	...
	InvertProcAmpParams		*infoP)
{
	...
	invert_params.exposure = infoP->mBrightness;
